...
document.addEventListener("DOMContentLoaded", ()=>{
    const josh = new _joshjsDefault.default({
        initClass: "josh-js",
    ...
    document.getElementById("load-more").addEventListener("click", function() {
        ...
        div.style = "background: #ffe9c5; color: #000; border-bottom: 3px solid #fff;";
...
class Josh {
    ...
    intersectionObserverCallback(entries, observer) {
        ...
        entries.forEach((entry)=>{
            ...
            const name = targetElm.dataset.joshAnimName, iteration = targetElm.dataset.joshIteration, duration = targetElm.dataset.joshDuration, delay = targetElm.dataset.joshDelay; // Animation delay
        ...
        `;
                if (!notMobile) targetElm.style = targetElm.style.cssText + styles;
                //Removing observer
    ...
    //Adding css to DOM
    addCss(targetElm) {
        if (targetElm.length > 0) targetElm.forEach((elm)=>{
            this.cssUtil(elm);
        });
        else this.cssUtil(targetElm);
    }
    //Css utility
    cssUtil(targetNode) {
        const notMobile = !this.animateInMobile && this.isMobile();
        targetNode.classList.add(this.animClass);
        if (!notMobile) // Initial time content will not show if that content not in the view.
    ...
    intersectOnScroll(domElement) {
        ...
        if ("IntersectionObserver" in window) {
            ...
            if (domElement.length > 0) //Observing every element
            domElement.forEach((animItem)=>{
                intetsectObserver.observe(animItem);
            });
            else //Observing single element
            intetsectObserver.observe(domElement);
        } else //If IntersectionObserver not supported by browser thowing error
            ...
            if (this.onDOMChange) {
                ...
                const domChangeObserver = new MutationObserver((mutations)=>{
                    mutations.forEach((mutation)=>{
                        ...
                        if (newNodes !== null) newNodes.forEach((node)=>{
...
},{}]},["d5kvp","igcvL"], "igcvL", "parcelRequireca8c")
