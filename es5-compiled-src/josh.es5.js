 ...
 */
 export default class Josh {
  constructor(options = {}) {
    ...
    this.initClass = options.initClass || "josh-js"; // Selecting DOM element

    this.initDom = document.querySelectorAll("." + this.initClass); // Animation library class. Default class is 'animate__animated'

    this.animClass = options.animClass || "animate__animated"; // Number 0 to 1, 0.2 means if the element come to 20% percent of view it will animate. Default value is 0.2

    this.offset = options.offset || 0.2; // Boolean value to animate in mobile or not. Default value is true

    this.animateInMobile = typeof options.animateInMobile === "undefined" ? true : options.animateInMobile; //To run on DOM change you need to pass the true value. Default value is false

    this.onDOMChange = typeof options.onDOMChange === "undefined" ? false : options.onDOMChange; //Call intersectOnScroll method on constructor run

    this.intersectOnScroll(this.initDom); //Call callOnDOMChange method on constructor run

    this.callOnDOMChange(); //Adding css in constructor run
  ...
  intersectionObserverCallback(entries, observer) {
    ...
    entries.forEach(entry => {
      ...
      const targetElm = entry.target; //Taking data from data attribute
  ...
  intersectOnScroll(domElement) {
    ...
    if ("IntersectionObserver" in window) {
      const intetsectObserver = new IntersectionObserver(this.intersectionObserverCallback.bind(this), {
        root: null,
        rootMargin: "0px",
